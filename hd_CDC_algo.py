...
class Initial_hdcdc:
    ...
    def findTotalCourses(self):
        ...
        self.totalCourses = 0
        for i in range(n):
            for j in range(len(self.faculties[i].hd_cdc_pref)):
                self.totalCourses = max(self.totalCourses, self.faculties[i].hd_cdc_pref[j][0])
    ...
    def generateMatrix(self):
        ...
        for i in range(len(self.faculties)):
            for j in range(len(self.faculties[i].hd_cdc_pref)):
                self.matrix[i][self.faculties[i].hd_cdc_pref[j][0] - 1] = -1 
        # print(self.matrix)
